% Demo on non_i.i.d. Gaussian + stripe Noise
clear,clc
currentFolder = pwd;
...
for num = 1:20
    ...
    band = ceil((B-20)*rand(1,40)+10);               % bands chose to add stripe noise 
    stripnum = 20+ceil(20*rand(1,length(band)));     % number of stripe in these bands   
    for i=1:length(band)
        loc = ceil(N*rand(1,stripnum(i)));
        t = rand(1,length(loc))*0.5-0.25;
        Noi_H(:,loc,band(i)) = bsxfun(@minus,Ori_H(:,loc,band(i)),t);
    end
